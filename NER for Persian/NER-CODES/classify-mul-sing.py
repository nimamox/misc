...
NN = Sequential()
NN.add(Dense(300 + len(whole_tags), 200, init='uniform'))
...
for si in xrange(len(sentences_samples)):
    ...
    for wi in xrange(len(sentences_samples[si])):
        ...
        tags_vec[whole_tags.index(sentences_samples_tag[si][wi])] = 5
        
        X.append(np.concatenate([prev, sentences_samples[si][wi], post, tags_vec]))
        prev = sentences_samples[si][wi]
        targ = [0, 0, 0, 0, 0, 0, 0]
...
NN.fit(X_train, y_train, nb_epoch=60, batch_size=10, show_accuracy=True, verbose=2)
